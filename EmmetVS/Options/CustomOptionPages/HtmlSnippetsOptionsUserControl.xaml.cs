...
/// <summary>
/// Interaction logic for HtmlSnippetsOptionsUserControl.xaml
/// </summary>
public partial class HtmlSnippetsOptionsUserControl : UserControl
{
    public ObservableCollection<KeyValuePair<string, string>> SettingsDict { get; set; }

    public HtmlSnippetsOptionsUserControl()
    {
        InitializeComponent();

        SettingsDict = new ObservableCollection<KeyValuePair<string, string>>(HtmlOptions.Instance.Snippets);
    ...
    private async void AddEventHandler(object sender, bool isCancelled)
    {
        try
        {
            ...
            if (!isCancelled)
            {
                ...
                {
                    var currentSnippets = HtmlOptions.Instance.Snippets;
                    currentSnippets.Add(dialogControl.Key, dialogControl.Value);
                    HtmlOptions.Instance.Snippets = currentSnippets;
