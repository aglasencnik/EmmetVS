...
/// <summary>
/// Interaction logic for XslSnippetsOptionsUserControl.xaml
/// </summary>
public partial class XslSnippetsOptionsUserControl : UserControl
{
    public ObservableCollection<KeyValuePair<string, string>> SettingsDict { get; set; }

    public XslSnippetsOptionsUserControl()
    {
        InitializeComponent();

        SettingsDict = new ObservableCollection<KeyValuePair<string, string>>(XslOptions.Instance.Snippets);
    ...
    {
        var dialogControl = new SnippetEntryDialog(actionButtonText: "Add");
        dialogControl.DialogResultEvent += AddEventHandler;

        var dialogWindow = new Window()
        {
            Title = "Add new XSL snippet",
    ...
    private async void AddEventHandler(object sender, bool isCancelled)
    {
        try
        {
            ...
            if (!isCancelled)
            {
                ...
                {
                    var currentSnippets = XslOptions.Instance.Snippets;
                    currentSnippets.Add(dialogControl.Key, dialogControl.Value);
                    XslOptions.Instance.Snippets = currentSnippets;
    ...
    private void OnEdit(object sender, RoutedEventArgs e)
    {
        ...
        var dialogControl = new SnippetEntryDialog(key: selectedKey, isKeyEnabled: false, value: selectedValue, actionButtonText: "Save");
        dialogControl.DialogResultEvent += EditEventHandler;

        var dialogWindow = new Window()
        {
            Title = "Edit XSL snippet",
            Content = dialogControl,
            SizeToContent = SizeToContent.WidthAndHeight,
            ResizeMode = ResizeMode.CanResizeWithGrip,
            WindowStartupLocation = WindowStartupLocation.CenterScreen
    ...
    private void OnDelete(object sender, RoutedEventArgs e)
    {
        ...
        var dialogControl = new SnippetEntryDialog(selectedKey, false, selectedValue, false, "Delete");
        dialogControl.DialogResultEvent += DeleteEventHandler;

        var dialogWindow = new Window()
        {
            Title = "Delete XSL snippet",
            Content = dialogControl,
            SizeToContent = SizeToContent.WidthAndHeight,
            ResizeMode = ResizeMode.CanResizeWithGrip,
            WindowStartupLocation = WindowStartupLocation.CenterScreen
